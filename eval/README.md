...
- `npm run eval:bench [-- --model M --runs N]` — live recall/precision vs PLANTED.md
  via `claude -p … --findings-json`. Needs `claude` on PATH; costs tokens. Writes
  `eval/results/<iso>-<model>.json`.
